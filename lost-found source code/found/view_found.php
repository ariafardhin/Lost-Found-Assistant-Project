    ...
    <?php while ($row = $result->fetch_assoc()): ?>
        <div class="item">
            ...
            <?php if ($row['user_id'] == $user_id): ?>
                <a href="edit_found.php?id=<?= $row['id'] ?>" class="btn">Edit</a>
                <a href="delete_found.php?id=<?= $row['id'] ?>" class="btn" onclick="return confirm('Are you sure you want to delete this item?');">Delete</a>
            <?php else: ?>
                <form action="../claims/claim_item.php" method="POST" class="claim-form" style="display:inline;">
                    ...
                    <button type="submit" class="btn">Claim This Item</button>
                </form>
            <?php endif; ?>
        </div>
    <?php endwhile; ?>
